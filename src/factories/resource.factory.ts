...
export class ResourceFactory {
    static create<T>(
        ...
        if (!isResource) throw new Error('Resource class must be decorated with @Resource');

        // Create an instance of the resource class
        const resource: T | undefined = ResourceClass.createInstance?.(...args);
        if (!resource) throw new Error('Failed to create resource instance');
        ...
        if (!basePath) throw new Error('Resource class must have a base path');
        ...
        if (!pathsInfo?.length) throw new Error('Resource class must have at least one path');
        ...
        for (const { path, method, handlerName } of pathsInfo) {
            ...
            if (!isFunction(requestHandler)) throw new Error(`Handler ${handlerName} is not a function`);
